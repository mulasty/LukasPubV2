  ...
  return (
    <StorySection sceneClassName="grid gap-8 lg:grid-cols-[0.98fr_1.02fr] lg:items-center">
      <div className="relative order-2 lg:order-1" data-reveal>
        <p className="section-kicker">Dance floor moment</p>
        <AnimatedText
          as="h2"
          text="Piątek odpala noc, sobota podkręca puls"
          ...
          {events.map((event) => (
            <article key={event.day} data-card className="glass-card rounded-[2rem] p-6">
              <div className="flex flex-wrap items-center gap-3">
                <span className="rounded-full border border-cyan-300/25 px-3 py-1 text-xs font-semibold uppercase tracking-[0.28em] text-cyan-200">
                  {event.day}
                </span>
      ...
      <div className="relative order-1 min-h-[560px] lg:order-2" data-reveal>
        <div data-parallax data-speed="-0.14" className="absolute left-[5%] top-[8%] h-48 w-48 rounded-full bg-violet-500/18 blur-3xl" />
        ...
        >
          <div className="mb-5 flex items-center justify-between gap-4">
            <p className="text-xs font-semibold uppercase tracking-[0.34em] text-cyan-200">Bar preview</p>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            {menuSections.slice(0, 2).map((section) => (
              <div key={section.category} className="rounded-[1.5rem] border border-white/8 bg-white/5 p-4">
                <p className="text-xs font-semibold uppercase tracking-[0.28em] text-fuchsia-300">{section.category}</p>
                <div className="mt-4 space-y-3">
                  {section.items.slice(0, 3).map((item) => (
                    <div key={item.name} className="flex items-start justify-between gap-3 text-sm">
                      ...
                      <span className="text-cyan-200">{item.price}</span>
                    </div>
                  ))}
                </div>
              </div>
